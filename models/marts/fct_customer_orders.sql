...
customer_orders as (
  ...
  select 
    ...
    sum(case when orders.valid_order_date is null then 0 else 1 end) over(
      partition by orders.customer_id
    ) as customer_non_returned_order_count,

    sum(case when orders.valid_order_date is null then 0 else orders.order_value_dollars end)  over(
      partition by orders.customer_id
